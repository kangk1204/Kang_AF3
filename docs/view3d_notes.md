...
> **이 표는 2026-08-20 기준이고, 그 다음 날 추가된 CSP 이후의 페이지를 설명하지
> 않는다.** CSP 는 2026-08-21 커밋 `03696f2` 에서 들어왔고 `'unsafe-eval'` 이 없어
> molstar 를 막았다. 즉 위 결과는 CSP 가 없던 HTML 로 얻은 것이다. 2026-08-25 에
> 실제 Chrome 으로 다시 확인한 결과는 6-7 절에 있다.
...
### 6-7. 2026-08-25 재검증 (사람이 쓰는 실제 Chrome)

원격 리눅스 작업기에서 만든 HTML 을 로컬 macOS 의 Chrome 으로 열어 확인했다.
헤드리스도 소프트웨어 WebGL 도 아니다. 사람이 쓰는 그 브라우저다.

**먼저 찾은 것.** 커밋된 예시가 브라우저에서 구조를 전혀 못 띄웠다.
    ...
    allowed source of script
        at new Function (...)  at molstar.js:4:21182

molstar 번들이 초기화 중 `new Function(...)` 을 부른다. `'unsafe-eval'` 이 없으면
거기서 죽고, 페이지에는 지표 표 옆에 "molstar 전역이 없다" 만 남는다. HTML 문법,
CDN 핀, SRI 해시, 데이터가 전부 정상이라 **정적 검사로는 잡히지 않는다.**
6-3 의 렌더 확인이 CSP 도입(2026-08-21) 하루 전이라 아무도 다시 보지 않았다.

**고친 것 세 가지.**

| 결함 | 원인 | 고친 것 |
| --- | --- | --- |
| 구조가 아예 안 뜬다 | CSP `script-src` 에 `'unsafe-eval'` 이 없다 | 허용하되 출처는 CDN 두 곳 + SRI 로 그대로 잠가 둔다 |
| 실패 안내가 원인을 잘못 짚는다 | 우리 코드에는 '전역이 없다' 로만 보여 네트워크를 탓했다 | `securitypolicyviolation` 을 라이브러리 로드 전에 받아 두고, 막힌 지시어를 안내에 그대로 보여 준다 |
| '시점 초기화' 가 처음 각도로 안 돌아간다 | `requestCameraReset()` / `zoomTo()` 는 확대·중심만 맞추고 회전은 그대로 둔다 | 자동 프레이밍이 **멈춘 뒤** 의 시점을 저장해 두고 그것을 복원한다 |

`connect-src 'none'` 도 고쳤다. molstar 는 초기화 중 `data:` URI 를 fetch 하므로
모든 페이지에 위반이 하나씩 계속 쌓이고 있었다. 화면은 정상이라 눈에 띄지 않지만,
위반이 늘 남아 있으면 위의 새 안내가 **무관한 실패까지 CSP 탓으로** 돌리게 된다.
`connect-src data:` 로 허용했다 (data: 는 바깥으로 나가지 않는다). 그리고 안내는
라이브러리를 실제로 못 싣게 하는 지시어(`script-src`, `style-src`, `worker-src`,
`child-src`, `default-src`)의 위반만 원인으로 본다. 고친 뒤 위반은 0건이다.

시점 초기화는 고치는 과정에서 두 번 더 틀렸고, 둘 다 실제 브라우저에서만 드러났다.
고정 지연(800ms)으로 시점을 찍으면 프레이밍이 진행 중인 카메라를 집어서, 복원했을 때
구조가 잘려 거의 안 보였다. 그리고 Mol\* 은 `camera.setState()` 를 무시한다
(trackball 이 매 프레임 자기 상태를 다시 얹는다). 값이 두 번 연속 같아질 때까지
기다렸다 찍고, `requestCameraReset({snapshot})` 으로 돌려줘야 한다.

**확인 범위.** 네 조합과 목록 페이지를 전부 실제로 열었다.

| 페이지 | 엔진 | 라이브러리 | 결과 |
| --- | --- | --- | --- |
| 복합체 | Mol\* 5.11.0 | cdn | 구조 표시, 콘솔 오류 0 |
| 복합체 | Mol\* 5.11.0 | embed | 구조 표시 |
| 복합체 | 3Dmol.js 2.5.5 | cdn | 구조 표시 |
| 복합체 | 3Dmol.js 2.5.5 | embed | 구조 표시 |
| 단량체 vhh_7mfv_1 | Mol\* 5.11.0 | cdn | 구조 표시, ipTM 행 없음 |
| `index.html` | - | - | 2건이 ranking score 내림차순, 단량체 ipTM 은 `-`, 이름을 눌러 이동됨 |
| `examples/view3d_example.html` | Mol\* 5.11.0 | cdn | 구조 표시 |

- **마우스로 직접 돌렸다.** 드래그로 회전이 되고 축 표시가 따라 돌았다.
- **'사슬별' 버튼을 눌렀다.** Mol\* 은 A 초록/B 주황, 3Dmol 은 A 파랑/B 주황이고
  범례 견본이 화면 색과 같았다.
- **'시점 초기화' 버튼을 눌렀다.** 고친 뒤에는 처음 화면으로 정확히 돌아온다.
  3Dmol 은 `getView()` 로 되읽어 `[0.23,0.05,0.77,-286.72,0,0,0,1]` 이 복원됨을 확인했다.
- **cdn 과 embed 는 엔진 코드가 같다.** 두 빌드의 엔진 script 블록이 바이트 단위로
  일치한다 (molstar 3539자 / 3Dmol 2233자, SHA-256 앞 16자 동일). `--lib` 는
  라이브러리를 어디서 싣는지만 정한다. 그래서 버튼·카메라 확인을 cdn 에서 한 것이
  embed 에도 그대로 적용된다. 이 불변식은 회귀 테스트로 고정했다.

**커밋된 예시를 다시 만들었다.** 기존 `examples/view3d_example.html` 은 mmCIF 를
base64 가 아니라 **평문으로** 담고 있었다. base64 전환과 같은 커밋(`f73578f`)에
들어왔지만 예시 파일은 그 전 버전으로 만들어진 것이었다. 즉 커밋되는 순간부터
낡아 있었다. 손으로 깁지 않고 실제 실행 결과로 다시 생성했다
(`examples/vhh_antigen_complex.json`, 축소 DB 오버레이 + 전체 DB 폴백,
`--num_diffusion_samples` 기본 5, ranking score 0.850 / pTM 0.730 / ipTM 0.850).
`tests/test_reporting.py` 가 이제 예시와 생성기 껍데기의 일치를 강제하므로
다시 낡으면 테스트가 실패한다.
...
- **Safari, Firefox, Edge 에서 열어 보지 못했다.** 2026-08-25 에 일반
  데스크톱 Chrome(macOS, GPU 있음)으로는 확인했다 (6-7 절). 나머지 브라우저는
  Mol* 과 3Dmol 이 공식 지원하지만 이 저장소가 만든 HTML 로 확인한 것은 아니다.
  특히 CSP 의 `'unsafe-eval'` 처리는 브라우저마다 다를 여지가 있다.
- **확대와 이동(pan)은 따로 눌러 보지 않았다.** 2026-08-25 에 마우스 드래그
  회전과 버튼 세 개는 실제로 확인했다 (6-7 절). 확대/이동은 라이브러리 기본
  동작이고 우리가 손대지 않았지만, 직접 해 본 것은 아니다.
