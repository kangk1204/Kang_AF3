...
| 질문 | 답하는가 |
|---|---|
| 이 서열이 어떤 구조로 접히는가 | **예측한다** |
| 두 사슬이 어떤 자세로 붙을 수 있는가 | **예측한다** |
| 두 단백질이 실제로 결합하는가 | **아니다** |
| 결합 세기(Kd, IC50) | **아니다** |
| 돌연변이가 결합을 얼마나 바꾸는가 | **아니다.** 두 예측의 점수 차이는 결합 변화량이 아니다 |

**ipTM 이 높다는 것은 AF3 가 그 자세에 확신한다는 뜻이지, 결합한다는 뜻이 아니다.**
AF3 는 주어진 사슬을 어떻게든 배치한다. 결합 여부와 세기는 실험이 판단한다.
...
| 모델 가중치 | 공식 Google 경로에서 직접 내려받는 것이 가장 단순하다. 현재 약관은 같은 조직 안의 조건부 공유만 예외로 두므로, 조직 밖·공개 저장소로 공유하지 않는다 |
