...
| 디스크 | 약 1TB 여유. 데이터베이스만 850GB 다 |
| 모델 가중치 | 본인이 **직접 내려받는다**. 신청·승인 절차는 없다. 동료에게 복사받으면 약관 위반이다 |
| 시간 | 설치와 DB 내려받기에 수 시간 |

가중치 약관은 비영리 한정이고 재배포가 금지돼 있다. 신청서를 내는 절차는 없고,
**받아서 쓰는 행위 자체가 동의**로 간주된다. 그러니 받기 전에 약관을 한 번 읽고,
내려받은 날짜를 적어 둔다. 나중에 논문 심사나 기관 감사에서 근거가 된다. 자세한 것은
...
지금까지 대조한 복합체 7건에서 **등급이 뒤집힌 적은 없다.** 그래서 거르는 용도로는
overlay 를 쓴다. 다만 7건은 규칙을 세우기에 적은 수다.
