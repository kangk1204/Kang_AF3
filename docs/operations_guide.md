...
시드를 늘리거나 샘플 수를 바꿔 다시 돌릴 때 MSA 재계산을 건너뛸 수 있다.
**절약폭은 1단계에 쓴 DB 구성이 정한다.** 축소 DB 약 2GB 라면 데이터 파이프라인이
건당 1.98초뿐이므로 시간 절약은 건당 1.7~5.3초로 작다. 전체 DB 급(4GB 슬라이스 4종)
이라면 건당 약 30초를 아낀다.
**축소 DB 에서 재사용을 권하는 진짜 이유는 시간이 아니라 1단계와 2단계가 완전히 같은
MSA 를 쓴다는 것이다.** 같은 축소 DB 로 MSA 를 새로 만들면 unpaired 깊이가
9→7, 13→10, 12→9 로 달라지는 것이 실측됐다 (`docs/two_stage_notes.md` 3절).
```bash
...
건당 시간의 대부분은 **1단계에 쓰는 DB 구성**이 정한다. 아래 두 구성은 측정 조건이 달라
직접 비교할 수 없다. 각 행에 조건과 근거를 붙였다.

| 단계 / 구성 | 건당 | 2000건 | 근거 |
|---|---|---|---|
| 현재 방식 (연구자 보고, 5090 + Docker, 타깃당 재기동) | 341초 | 189시간 (7.9일) | 연구자 보고 |
| GPU 추론만 (단일 프로세스 + 캐시) | 5.39초 | 3.0시간 | 직접측정 |
| 데이터 파이프라인: 축소 DB 약 2GB | 1.98초 | 1.1시간 | 직접측정 (VHH 4건) |
| 데이터 파이프라인: 전체 DB 급 4GB 슬라이스 4종 | 30.41초 | 16.9시간 | 직접측정 (같은 4건) |
| MSA 단계 포화 처리율 0.895 타깃/분, 전체 DB 급 4GB 슬라이스 4종 | 67.0초 | 37.2시간 | 인용 (14조합 스레드 스윕) |
| **합계: 축소 DB 구성 (권장)** | 7.4초 | **4.1시간** | 합산 추정 |
| **합계: 전체 DB 급 구성** | 72.4초 | **40.2시간 (1.7일)** | 합산 추정 |

**축소 DB 로 스크리닝하는 권장 구성에서는 MSA 가 병목이 아니다.** 데이터 파이프라인
건당 1.98초가 GPU 추론 5.39초보다 짧다. **MSA 가 전체의 93%를 차지하는 것은 전체 DB 급
구성에서만 성립한다.** 어느 쪽이든 코드를 더 손대서 줄어드는 부분은 아니고, 줄이려면
MSA 를 재사용하거나 사전 계산하는 방향이다 (3-6절). 조건별 대조 측정은
`docs/two_stage_notes.md` 3절, 정정 경위는 `docs/msa_correction_notes.md`.
