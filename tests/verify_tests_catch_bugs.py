...
INJECTIONS = [
    ...
    },
    {
        ...
        "script": "run_af3_batch_improved.py",
        "old": """                if name.endswith(".json") and not name.startswith("._")""",
    ...
    {
        "name": "AF3 이미지 버전 검증 실패를 무시한다",
        "detail": "AF3 image version assertion이 실패해도 설치가 계속된다.",
        "script": "install_af3_ubuntu.sh",
        "old": '    >/dev/null || die "AF3 image does not report the pinned version $AF3_VERSION: $IMAGE"',
        "new": '    >/dev/null || true',
    ...
    },
    {
        ...
        'script': 'af3_batch.py',
        'old': '        output_dir / ".run_af3_batch.lock",  # common with the preferred runner',
        'new': '        work / ".run_af3_batch.lock",  # BUG: shared output is no longer locked',
    ...
    },
    {
        ...
        'old': '            if internal != wanted_name:',
        'new': '            if False:',
        'tests': ['test_stage2_source_requires_regular_unique_internal_identity'],
    },
    {
        'name': '선정내역 CSV 의 symlink 방어를 없앤다',
        'detail': '결과 폴더에 링크를 심어 두면 바깥 파일이 덮어써진다.',
        'script': 'af3_stage2.py',
        'old': '    return _AtomicManifest(path)',
        'new': '    return open(path, "w", encoding="utf-8-sig", newline="")',
    ...
    },
    {
        ...
        'script': 'af3_check.sh',
        'old': 'assert backend == "gpu"; assert devices and all(device.platform == "gpu" for device in devices)',
        'new': 'assert devices',
